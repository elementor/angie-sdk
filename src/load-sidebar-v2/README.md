...
| Section | Purpose |
|---------|---------|
| `host` | **Required.** `appId`, optional `instanceId` (see [multiple instances](#multiple-instances-on-one-page)), `aiContext`, `website`, `analytics`, optional `generationTypes` (`string[]` forwarded as-is; the SDK does not interpret ids) sent to the embedded Angie app (see [aiContext](#hostaicontext)) |
...
### host.generationTypes

Optional `string[]` on `host`, copied onto `sdk-embedded-config` / `HOST_READY` `embedded.generationTypes`. The SDK does not interpret values. Angie uses known ids (for example `artifacts`) and ignores unknown ones. Omit the field when the host does not opt into any generation kinds.
