...
export const openSaaSPage = async ( props: OpenSaaSPageInput ): Promise<OpenSaaSPageOutput> => {
	...
	return new Promise( ( resolve ) => {
		...
		const css = { 'background-color': 'transparent', 'color-scheme': 'normal', ...props.css };
		const onMessage = async ( event: MessageEvent ) => {
			if ( ! isTrustedIframeMessage( event, iframeUrlObject.origin, iframe ) ) return;
			switch ( event.data.type ) {
				case HostEventType.ANGIE_READY:
					resolve( { iframe, iframeUrlObject } );
					break;
				case HostEventType.ANGIE_LOADED: {
					...
					iframe.contentWindow?.postMessage( { type: HostEventType.HOST_READY, instanceId, ...( props.embeddedConfig ? { embedded: props.embeddedConfig } : {} ) }, iframeUrlObject.origin );
					break;
				}
				default: break;
			}
		...
		if ( props.insertCallback ) props.insertCallback( iframe ); else parent.body.appendChild( iframe );
	} );
};
