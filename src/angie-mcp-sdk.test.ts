...
describe('AngieMcpSdk', () => {
  ...
  describe('message handling', () => {
    ...
    it('should close and reconnect when server is already connected to a transport', async () => {
      // Arrange
      const mockPort = { postMessage: jest.fn() };
      ...
      // Assert - should close first, then reconnect
    ...
    it('should connect directly when server has no existing transport', () => {
      ...
      // Assert - should connect directly without close
      expect(mockServer.connect).toHaveBeenCalled();
    });
